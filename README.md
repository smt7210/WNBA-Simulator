...
## Data sources: SportsData.io WNBA API + The Odds API

SportsData.io remains the source for schedule/scores, team & player season stats, standings, and injuries. Get a key at [sportsdata.io](https://sportsdata.io) (WNBA API) and replace the placeholder in `config.js` (`SPORTSDATA_API_KEY`).

**Betting odds (moneyline, spread, total) now come from [The Odds API](https://the-odds-api.com)** (`odds-api-client.js`) instead of SportsData.io's `/odds` endpoint. Get a key at the-odds-api.com and replace the placeholder in `config.js` (`ODDS_API_KEY`). Notes:

- The Odds API's live odds endpoint returns the whole upcoming/live slate in one call (no per-date filter param), so the client fetches everything once per schedule load and filters to the selected date locally — keeps usage low against the free tier's 500 requests/month.
- The Odds API identifies teams by full name ("Las Vegas Aces"); SportsData.io's game rows use abbreviations ("LVA"). `odds-api-client.js` has an alias table mapping every current WNBA franchise's full name/city/nickname to this app's abbreviation so the two sources match up. If a team is missing (expansion/relocation), its odds fall back to "unavailable" rather than a guessed match.
- If The Odds API call fails (bad key, rate limit, market not on your plan), the app falls back to whatever odds SportsData.io's `GamesByDate` embedded for that game, so a game card doesn't go blank.
- **Backtest odds:** The Odds API's historical-odds endpoint requires a paid plan and costs 10x the request quota, so `backtest.js` tries it first and falls back to SportsData.io's historical odds on any failure — expect the free tier to always use the SportsData.io fallback for backtesting.
- `dataSourceStatus` in the UI now shows `oddsApi: OK/unavailable` alongside the SportsData.io feeds.
...
| `sportsdata-client.js` | SportsData.io WNBA API calls: schedule/scores, team/player season stats, standings, injuries, historical box scores for backtesting. (Historical odds fallback only - see `odds-api-client.js`.) |
| `odds-api-client.js` | The Odds API calls: live/upcoming betting odds (moneyline, spread, total) for the schedule view, plus a best-effort historical-odds call for backtesting on paid plans. Team full-name ↔ abbreviation alias table lives here. |
